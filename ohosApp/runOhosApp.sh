...
if [ -z "$targets" ]; then
  ...
  $HDC_BIN -t "$target_id" shell aa force-stop com.example.kuiklylibgallery
  $HDC_BIN -t "$target_id" install entry/build/default/outputs/default/entry-default-signed.hap
  $HDC_BIN -t "$target_id" shell aa start -a EntryAbility -b com.example.kuiklylibgallery
  done
fi